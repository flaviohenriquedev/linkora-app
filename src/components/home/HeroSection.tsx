  ...
  return (
    <div className="bg-[radial-gradient(circle_at_center,#152B1F_0%,transparent_70%)] pb-12 pt-12 text-center sm:pb-16 sm:pt-16 md:pt-20">
      <Container>
        ...
        <div className="flex flex-wrap justify-center gap-8 text-text-secondary sm:gap-10 md:gap-12">
          {[
            { v: "+5000", l: "Profissionais" },
            { v: "+25", l: "Áreas" },
            { v: "+400", l: "Empresas Conectadas" },
          ].map((s) => (
            <div key={s.l} className="min-w-[5.5rem] text-center">
              <div className="font-serif text-2xl font-semibold text-gold sm:text-3xl md:text-[32px]">
                {s.v}
              </div>
              <div className="mt-1 text-xs sm:text-sm">{s.l}</div>
            </div>
          ))}
        </div>
      </Container>
